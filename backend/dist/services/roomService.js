...
const mapRoom = (room) => ({
    ...
    participants: (room.participants || []).map((p) => typeof p === 'object' && p._id ? mapUser(p) : { id: p.toString() }),
...
export const roomService = {
    ...
    // Create new room
    async createRoom(name, hostId, topic, type = 'PUBLIC', maxUsers = 10) {
        try {
            const room = new Room({
                name,
                hostId,
    ...
    async addUserToRoom(roomId, user) {
        if (!isValidId(roomId) || !isValidId(user.id))
            return null;
        try {
            const room = await Room.findOneAndUpdate({
                _id: roomId,
                $expr: { $lt: [{ $size: "$participants" }, "$maxUsers"] }
            }, { $addToSet: { participants: user.id } }, { new: true }).populate('participants').lean();
            if (!room) {
    ...
    async removeUserFromRoom(roomId, userId) {
        if (!isValidId(roomId) || !isValidId(userId))
            return null;
        try {
            const room = await Room.findByIdAndUpdate(roomId, { $pull: { participants: userId } }, { new: true }).populate('participants').lean();
            return room ? mapRoom(room) : null;
        }
        catch (error) {
    ...
    // Add message to room
    async addMessageToRoom(roomId, userId, username, text, isSystem = false) {
        if (!isValidId(roomId))
            return null;
        try {
    ...
    async getRoomMessages(roomId) {
        ...
        try {
            ...
            if (!room)
                return [];
            return (room.messages || []).slice(-50).map((m) => ({
                id: m._id?.toString(),
                roomId: m.roomId,
                userId: m.userId,
                username: m.username,
    ...
    // Update active game
    async updateActiveGame(roomId, gameType) {
        if (!isValidId(roomId))
            return null;
        try {
    ...
    async updateUserPermissions(roomId, userId, permissions) {
        if (!isValidId(roomId) || !isValidId(userId))
            return null;
        try {
