...
const activeGameData = new Map<string, { gameType: string; gameData: any }>();
  ...
  io.on('connection', (socket: SocketUser) => {
    ...
    socket.on('join-room', async (data: { roomId: string }, callback) => {
      ...
      const systemMessage = await roomService.addMessageToRoom(
        data.roomId, 'system', 'System',
        `${socket.user.username} joined the room`, true
      );
    ...
    // Send message
    socket.on('send-message', async (data: { text: string }, callback) => {
      if (!socket.userId || !socket.roomId || !socket.user) {
        callback({ success: false });
        return;
      }

      const message = await roomService.addMessageToRoom(
        socket.roomId,
        socket.userId,
    ...
    socket.on('start-game', async (data: { gameType: string }, callback) => {
      if (!socket.roomId) {
        callback({ success: false });
        return;
      }

      const room = await roomService.updateActiveGame(socket.roomId, data.gameType as any);
      if (!room) {
        callback({ success: false });
        return;
      }
    ...
    socket.on('end-game', async (callback) => {
      if (!socket.roomId) { callback({ success: false }); return; }
    ...
    socket.on('quiz-answer', (data: { answerIndex: number }) => {
      if (!socket.roomId || !socket.userId || !socket.user) return;
    ...
    socket.on('request-media-permission', async (data: { type: 'audio' | 'video' }) => {
        if (!socket.roomId || !socket.userId || !socket.user) return;
    ...
    socket.on('grant-media-permission', async (data: { userId: string, type: 'audio' | 'video', allowed: boolean }) => {
        ...
        if (data.type === 'audio') permissions.canSpeak = data.allowed;
        if (data.type === 'video') permissions.canVideo = data.allowed;
